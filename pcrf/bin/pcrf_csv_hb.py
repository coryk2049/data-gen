...
def main(argv):
    ...
    if (system_id is not None) and \
        ...
        pm = pcrf_csv_hb(system_id,division_id,start_datetime,end_datetime)
        ec = [0,0]
        try:
            ...
            try:
                pm.edr_head(edr_fh)
                pm.ulog_head(ulog_fh)
                pm.stats_head(stats_fh)
